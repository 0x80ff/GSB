...
class DataAccess extends CI_Model
{
     ...
     */
    public function getInfosEmployes($login, $mdp)
    {
        $req   = "select employes.id as id, employes.nom as nom, employes.prenom as prenom, employes.fonction as fonction 
				from employes 
				where employes.login=? and employes.mdp=?";
    ...
    /**
     * Retourne les informations des fiches signées ou refusées
     * 
     * @return $lesLignes La liste des fiches
     */
    public function getLignesFortaitSign()
    {
        $req =  "select idVisiteur, nom, prenom, mois, idEtat, montantValide, dateModif, Commentaire
				from fichefrais, employes
				where 
				(fichefrais.idVisiteur = employes.id and idEtat = 'CL') or
				(fichefrais.idVisiteur = employes.id and idEtat = 'RE')
				order by idVisiteur";
     ...
     * @return l'id, le nom et le prénom sous la forme d'un tableau associatif 
     */
    public function getLignesFortaitVal()
    {
        $req = "select idVisiteur, nom, prenom, mois, idEtat, montantValide, dateModif, Commentaire
                rom fichefrais, employes
                where 
                (fichefrais.idVisiteur = employes.id and idEtat = 'MP') or
                (fichefrais.idVisiteur = employes.id and idEtat = 'VA') order by idVisiteur";
    ...
    {
        $req = "select idVisiteur, fichefrais.idEtat as idEtat, fichefrais.dateModif as dateModif, 
     ...
     */
    public function majEtatFicheFrais($idVisiteur, $mois, $etat, $commentaire = null)
    {
        $req = "update fichefrais 
				set idEtat = '$etat', dateModif = now(), Commentaire = '$commentaire'
				where fichefrais.idvisiteur ='$idVisiteur' and fichefrais.mois = '$mois'";
        $this->db->simple_query($req);
    }
